...
### 2026-01-20 22:00 - 批量导出与预览不一致问题分析

#### 问题现象

用户反馈批量导出的结果与预览界面完全不同，缺少滤镜、色彩调整、水印等效果。

#### 根本原因

批量导出 (`useBatchExport.processProject`) 和预览渲染 (`useTextureGenerator.processImage`) 使用的参数来源不一致：

| 参数 | 预览渲染 | 批量导出 | 差异 |
|------|---------|---------|------|
| **LUT** | `lutControl.lutFile.value` | ❌ 未传递 | 完全缺失 |
| **LUT 强度** | `lutControl.lutIntensity.value` | ❌ 未传递 | 完全缺失 |
| **globalHSL** | `buildHSLLayers()` 合并 | `project.params.hslLayers` 直接使用 | 全局调整丢失 |
| **水印** | `watermarkConfig.value` (项目状态) | `preset.value.watermarkConfig` (导出预设) | 来源不同 |

#### 设计缺陷

1. **`ProjectParams` 缺少 LUT 字段**：
   ```typescript
   interface ProjectParams {
       // ... 现有字段
       // ❌ 缺少以下字段：
       lutId?: string           // LUT 在 lutDb 中的 ID
       lutIntensity?: number    // LUT 强度
   }
   ```

2. **`useLUTControl` 未与项目状态同步**：与 `useAdjustmentParams` 不同，`useLUTControl` 是完全独立的状态，切换项目时不会自动恢复 LUT 选择。

3. **导出逻辑未调用 `buildHSLLayers`**：预览渲染会将 `globalHSL` 封装为 ID 为 `global` 的特殊图层，但导出逻辑直接使用 `hslLayers` 数组。
...
让 `useLUTControl` 像 `useAdjustmentParams` 那样与项目状态双向同步：
- 切换项目时，从 `activeProject.params.lutId` 加载 LUT
- 修改 LUT 时，调用 `updateProjectParams({ lutId, lutIntensity })`

##### 3. 统一渲染参数组装

创建 `buildRenderParams(project: ImageProject)` 工具函数：
- 从 `project.params` 读取基础参数
- 调用 `buildHSLLayers` 逻辑合并 `globalHSL`
...
```typescript
type WatermarkPriority = 'project' | 'global'

function resolveWatermarkConfig(
    projectConfig: 水印配置 | null,
    globalConfig: 水印配置 | null,
    priority: WatermarkPriority
): 水印配置 | null {
    if (priority === 'project') {
        // 项目优先：有项目配置用项目，没有用全局
...
导出时，用户可以在导出对话框中选择水印优先级。

---

### 2026-01-20 21:55 - LUT 预览切换不同步

#### 问题确认

切换项目时，LUT 选择状态没有随项目变更改变。这是因为 `useLUTControl` 是完全独立的状态管理 Composable，没有与 `projectState` 同步。

#### 修复优先级

- **高**：LUT 参数持久化与同步（影响预览和导出）
- **高**：globalHSL 合并逻辑（影响导出）
- **中**：水印优先级机制（需要 UI 支持选择优先级）
