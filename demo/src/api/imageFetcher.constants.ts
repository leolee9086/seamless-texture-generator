...
export const LOG_MESSAGES = {
  CACHE_HIT: (url: string): string => `[fetchImage] 缓存命中 URL: ${url}`,
  CACHE_MISS: (url: string): string => `[fetchImage] 缓存未命中，开始轮询 URL: ${url}`,
  ...
  FETCH_SUCCESS: '[fetchImage] 获取成功，开始缓存',
  CACHE_VALIDATION_SUCCESS: '[fetchImage] 缓存验证成功',
} as const
