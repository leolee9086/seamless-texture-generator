 ...
 */
// @AIDONE 已完成 common-utils 基础迁移
// - common-utils: 通用工具函数（已迁移 common.guard.ts, deviceDetection.ts, scroll.ts, scroll.guard.ts, vconsole.ts）
// - app-utils: 图像处理相关工具（待迁移）

// @AITODO 继续整理 utils 目录:
// 1. 迁移更多 common-utils 文件: vue/ 目录 (需评估是否与业务解耦)
// 2. 迁移 app-utils 文件: geometry.ts, homography.ts, imageHandlers.ts 等
