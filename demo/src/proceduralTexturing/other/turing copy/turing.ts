...
fn fs_main(in : VertexOutput) -> @location(0) vec4<f32> {
    let data = getSurfaceHeight(in.uv);
    let height = data.x;
    let mask = data.y; // 0 = Base Skin, 1 = Pigment
    
    // --- 1. 法线重构 (Perturb Normal) ---
    // 计算屏幕空间导数来获得法线 (适合实时预览)
    // 对于导出贴图，应该使用多次采样 getSurfaceHeight
    let dHx = dpdx(height);
    let dHy = dpdy(height);
    // 增强法线细节
    let normalStrength = u.normDetail * 10.0;
    let normal = normalize(vec3<f32>(-dHx * normalStrength, -dHy * normalStrength, 1.0));
    
    // --- 2. 多层皮肤着色 (Subsurface Scattering Fake) ---
    // 简单的 SSS 近似：
    // Base Color = Mix(真皮层, 表皮层)
    // "透光"区域通常在法线边缘或阴影处，这里简单模拟为颜色层叠
    
    let skinColor = mix(u.colorSub, u.colorEpi, 0.8); // 80% 表皮覆盖
    
    // 混合色素
    // 色素不仅仅是覆盖，它也有边缘消散
    let edge = smoothstep(0.0, 1.0, mask) - smoothstep(0.8, 1.0, mask); // 边缘带
    let finalAlbedo = mix(skinColor, u.colorPig, mask);
    
    // 在边缘处添加一点 SSS 辉光 (假装光线散射)
    // finalAlbedo += u.colorSub * edge * 0.2; 
    
    // --- 3. 材质属性 ---
    let roughness = mix(u.roughBase, u.roughPig, mask);
    // 也可以添加环境光遮蔽 (根据高度凹陷)
    let ao = smoothstep(0.0, 0.2, height + 0.1); 
    
    // --- 4. 影视级光照预览 (Image Based Lighting 近似) ---
    ...
    // Cook-Torrance Specular (简化版 GGX)
    let NdotH = max(dot(normal, H), 0.0);
    let alpha = roughness * roughness;
    let alpha2 = alpha * alpha;
    let denom = (NdotH * NdotH * (alpha2 - 1.0) + 1.0);
    let D = alpha2 / (3.14159 * denom * denom); // NDF
    let F = 0.04 + (1.0 - 0.04) * pow(1.0 - NdotH, 5.0); // Fresnel
    let spec = D * F * 2.0; // 强度调整
    
    // 漫反射 + 高光 + 环境光
    let ambient = vec3<f32>(0.03) * finalAlbedo * ao;
    let diffuse = finalAlbedo * NdotL * ao;
    
    let finalColor = ambient + diffuse + vec3<f32>(spec);
    
    // Gamma Correction
    let corrected = pow(finalColor, vec3<f32>(1.0/2.2));
