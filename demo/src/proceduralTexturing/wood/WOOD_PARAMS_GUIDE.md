...
1. **髓射线改进** (版本 2025-12-04)
   - 使用 `abs(rayNoise)` 生成对称条纹，取代原始噪声
   - 降低 `smoothstep` 阈值从 `(0.6, 1.0)` 到 `(0.3, 0.5)`
   - 增加默认 `rayFrequencyX` 从 50.0 到 80.0
   - 结果：射线更密集、更锐利、更真实

2. **导管改进** (版本 2025-12-04)
   - 增加频率倍数从 `1.0x` 到 `2.0x`
   - 增加 FBM octaves 从 2 到 3
   - 更改 `step(0.3)` 为 `smoothstep(0.85, 0.95)`
   - 降低影响强度从 0.5 到 0.15
   - 结果：孔隙更小、更密集、更离散，不再是大块区域
